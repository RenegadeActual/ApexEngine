...
static Key TranslateVirtualKey(WPARAM vk) {
    switch (vk) {
        ...
        default: return Key::Unknown; // Invalid key

    }
}

// ------------------------------------------------------------------------------
// Clear all input states. Called when window loses focus to prevent "stuck keys" when the user alt-tabs away with a key pressed.
// ------------------------------------------------------------------------------
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
        case WM_SYSKEYDOWN: {
            const bool wasDownBefore = (lParam & (1 << 30)) != 0; // check the "previous key state" bit to prevent auto-repeats from generating multiple "pressed" events
            if (!wasDownBefore) {
        ...
        // ----- Mouse Input -----
        case WM_LBUTTONDOWN:
            SetCapture(hwnd); // capture the mouse to continue receiving events even if the cursor leaves the window
        ...
        // ----- Mouse movement -----
        case WM_MOUSEMOVE: {
            // GET_X_LPARAM and GET_Y_LAPARAM handles the sign-extension for negative coordinates, which can happen when the cursor leaves the window in the top or left direction.
